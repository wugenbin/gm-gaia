...
public class PEMKeyManager extends X509ExtendedKeyManager {
	...
	private PEMKeyManager() {
		...
		try {
		    ...
		    reader = new PEMReader(new FileReader(HettyUtil.getFile(hettyConfig.getCertificateKeyFile())), new PasswordFinder() {
			...
			reader = new PEMReader(new FileReader(HettyUtil.getFile(hettyConfig.getCertificateFile())));
