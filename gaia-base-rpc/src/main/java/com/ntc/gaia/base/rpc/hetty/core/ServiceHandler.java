...
import com.ntc.gaia.base.rpc.spring.SpringContextUtil;
...
public class ServiceHandler {
		...
		if (service.getDefaultVersion() == null) {
			...
			if (iter.hasNext()) {
				...
				throw new HettyException(
						"your have a wrong in service config.check service[" + service.getName() + "]'s provider.");
			}
		}
	...
	public static void addToVersionMap(ServiceVersion version) {
		...
		if (!isServiceExits(version.getService())) {
			throw new HettyException(
					"please check your configure file,service[" + version.getService() + "] can't find.");
		}
	...
	public static Object handleRequest(RequestWrapper request) {
		...
		try {
			...
			Object processor = SpringContextUtil.getBean(serviceName);
